...
def bingx() -> pl.DataFrame:
    ...
    for market in res_swap.get("data", []):
        if not isinstance(market, dict):
            continue
        ...
        markets.append(
            MarketInfo(
                ...
                quote_currency=quote,
                price_precision=(
                    str(reverse_decimal_places(price_precision_val))
...
def bitget() -> pl.DataFrame:
    ...
    res_futures = market_http.get_futures_contracts(productType="USDT-FUTURES")
...
def kraken() -> pl.DataFrame:
    ...
    res_futures = market_http.get_futures_instruments(
