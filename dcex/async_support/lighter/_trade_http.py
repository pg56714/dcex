"""Lighter async signed trading HTTP client."""
...
class TradeHTTP(HTTPManager):
    ...
    async def _nonce(self, nonce: int | None = None, api_key_index: int | None = None) -> int:
    ...
    async def sign_create_order(
        ...
        return self._private_signer().sign_create_order(
            ...
            nonce=await self._nonce(nonce, api_key_index),
    ...
    async def create_order(
        ...
        return await self._signed_tx(
            await self.sign_create_order(
                market_index=market_index,
                client_order_index=client_order_index,
                base_amount=base_amount,
                price=price,
    ...
    async def cancel_all_orders(
        ...
        return await self._signed_tx(
            await self.sign_cancel_all_orders(
                time_in_force=time_in_force,
                timestamp_ms=timestamp_ms,
                skip_nonce=skip_nonce,
                nonce=nonce,
    ...
    async def update_leverage(
        ...
        return await self._signed_tx(
            await self.sign_update_leverage(
                market_index=market_index,
                fraction=fraction,
                margin_mode=margin_mode,
                skip_nonce=skip_nonce,
