...
[Arcus](https://docs.arcus.xyz/) perpetuals use separate Mainnet and Testnet
credentials (ARCUS_MAINNET_* / ARCUS_TESTNET_*). Pass testnet=True to select
Testnet. The signing key is the Ed25519 private seed generated by the Arcus
app; its public key is the API key. Stock perpetuals appear in the product
table as AAPL-USD-SWAP-style symbols. Arcus stock-token spot RFQ uses a
separate router and is not supported by this perpetuals client. A place-order
REST response acknowledges submission, not a confirmed fill.
The address-scoped WebSocket channels (orders and fills) are publicly readable
on Arcus and need no API key; subscribe to them to observe final order state.
