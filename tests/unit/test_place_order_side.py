...
Each sync trade client is built with a fake product-table manager and a stubbed
_request, then the side that would be sent in the payload is asserted. Covers
backward compatibility (legacy strings / hard-coded wrappers) and the OrderSide
enum, with no network and no API keys.
