# ruff: noqa: ANN001, ANN201, ANN202, D100, D103

import os
import time
from contextlib import suppress
from decimal import ROUND_DOWN, ROUND_UP, Decimal

import pytest
from dotenv import load_dotenv

from dcex.kraken.client import Client
from dcex.utils.errors import FailedRequestError

load_dotenv()

KRAKEN_SPOT_API_KEY = os.getenv("KRAKEN_SPOT_API_KEY")
KRAKEN_SPOT_API_SECRET = os.getenv("KRAKEN_SPOT_API_SECRET")
KRAKEN_FUTURES_API_KEY = os.getenv("KRAKEN_FUTURES_API_KEY")
KRAKEN_FUTURES_API_SECRET = os.getenv("KRAKEN_FUTURES_API_SECRET")
SPOT_QUOTES = ("USDT", "USDC", "USD")
FUTURES_SYMBOL = "BTC-USD-SWAP"
FUTURES_EXCHANGE_SYMBOL = "PF_XBTUSD"
SPOT_TRANSFER_AMOUNT = Decimal("1")
FUTURES_TRANSFER_AMOUNT = Decimal("0.1")

pytestmark = [
    pytest.mark.private,
    pytest.mark.stateful,
    pytest.mark.skipif(
        os.getenv("RUN_LIVE_TRADING_TESTS") != "1",
        reason="Set RUN_LIVE_TRADING_TESTS=1 to run real Kraken order and transfer tests.",
    ),
]


@pytest.fixture
def client():
    client_instance = Client(
        spot_api_key=KRAKEN_SPOT_API_KEY,
        spot_api_secret=KRAKEN_SPOT_API_SECRET,
        futures_api_key=KRAKEN_FUTURES_API_KEY,
        futures_api_secret=KRAKEN_FUTURES_API_SECRET,
    )
    _cleanup_state(client_instance)
    try:
        yield client_instance
    finally:
        _cleanup_state(client_instance)
        client_instance.close()


def _dec(value, default="0") -> Decimal:
    if value is None or value == "":
        value = default
    return Decimal(str(value))


def _fmt(value: Decimal) -> str:
    return format(value.normalize(), "f")


def _round_to_step(value: Decimal, step: Decimal, rounding: str) -> Decimal:
    if step <= 0:
        return value
    return (value / step).to_integral_value(rounding=rounding) * step


def _assert_spot_ok(response):
    assert isinstance(response, dict)
    assert response.get("error", []) == []
    assert "result" in response
    return response


def _assert_futures_ok(response):
    assert isinstance(response, dict)
    assert response.get("result") == "success", response
    return response


def _spot_balances(client: Client) -> dict:
    result = _assert_spot_ok(client.get_spot_account_balance()).get("result", {})
    return result if isinstance(result, dict) else {}


def _spot_available(client: Client, asset: str) -> Decimal:
    aliases = {"BTC": ("XXBT", "XBT", "BTC"), "USD": ("ZUSD", "USD")}
    balances = _spot_balances(client)
    for name in aliases.get(asset, (asset,)):
        if name in balances:
            return _dec(balances[name])
    return Decimal("0")


def _spot_details(client: Client, product_symbol: str) -> tuple[Decimal, Decimal, Decimal, Decimal]:
    details = client.ptm.get_trading_details("kraken", product_symbol)
    return (
        _dec(details.get("price_precision"), "0.01"),
        _dec(details.get("size_precision"), "0.00000001"),
        _dec(details.get("min_size"), "0.00001"),
        max(_dec(details.get("min_notional"), "1"), Decimal("1")),
    )


def _spot_prices(client: Client, product_symbol: str) -> tuple[Decimal, Decimal]:
    result = client.get_spot_orderbook(product_symbol, count=5).get("result", {})
    assert isinstance(result, dict) and result
    book = next(iter(result.values()))
    return _dec(book["bids"][0][0]), _dec(book["asks"][0][0])


def _spot_symbol_with_funds(client: Client) -> tuple[str, str, Decimal, Decimal, Decimal]:
    for quote in SPOT_QUOTES:
        if _spot_available(client, quote) <= 0:
            continue
        product_symbol = f"BTC-{quote}-SPOT"
        with suppress(Exception):
            client.ptm.get_exchange_symbol("kraken", product_symbol)
            _, ask = _spot_prices(client, product_symbol)
            _, step, min_size, min_notional = _spot_details(client, product_symbol)
            volume = max(
                _round_to_step(min_notional * Decimal("1.01") / ask, step, ROUND_UP),
                min_size,
            )
            if _spot_available(client, quote) >= volume * ask * Decimal("1.01"):
                return product_symbol, quote, step, min_size, volume
    pytest.fail("Insufficient Kraken spot quote balance for BTC spot order tests.", pytrace=False)


def _spot_limit_buy_params(client: Client) -> tuple[str, str, str]:
    product_symbol, quote, _, _, _ = _spot_symbol_with_funds(client)
    tick, step, min_size, min_notional = _spot_details(client, product_symbol)
    bid, _ = _spot_prices(client, product_symbol)
    price = _round_to_step(min(bid - tick, bid * Decimal("0.999")), tick, ROUND_DOWN)
    volume = max(
        _round_to_step(min_notional * Decimal("1.01") / price, step, ROUND_UP),
        min_size,
    )
    if _spot_available(client, quote) < price * volume:
        pytest.fail("Insufficient Kraken spot quote balance for post-only order.", pytrace=False)
    return product_symbol, _fmt(volume), _fmt(price)


def _spot_high_sell_price(client: Client, product_symbol: str) -> str:
    tick, _, _, _ = _spot_details(client, product_symbol)
    _, ask = _spot_prices(client, product_symbol)
    return _fmt(_round_to_step(ask + tick, tick, ROUND_UP))


def _spot_txid(response) -> str:
    txids = _assert_spot_ok(response).get("result", {}).get("txid", [])
    assert txids, response
    return str(txids[0])


def _cancel_spot(client: Client, txid: str) -> None:
    try:
        _assert_spot_ok(client.cancel_spot_order(txid=txid))
    except FailedRequestError as exc:
        if "Unknown order" not in str(exc):
            raise
    time.sleep(0.5)


def _skip_if_spot_open_orders(client: Client) -> None:
    _cleanup_spot_state(client, Decimal("0"))


def _futures_accounts(client: Client) -> dict:
    accounts = _assert_futures_ok(client.get_futures_accounts()).get("accounts", {})
    return accounts if isinstance(accounts, dict) else {}


def _futures_flex_available(client: Client) -> Decimal:
    flex = _futures_accounts(client).get("flex", {})
    return _dec(flex.get("availableMargin")) if isinstance(flex, dict) else Decimal("0")


def _futures_cash_available(client: Client, unit: str) -> Decimal:
    cash = _futures_accounts(client).get("cash", {})
    balances = cash.get("balances", {}) if isinstance(cash, dict) else {}
    item = balances.get(unit.lower(), {}) if isinstance(balances, dict) else {}
    if not isinstance(item, dict):
        return Decimal("0")
    for key in ("available", "balance", "amount"):
        if key in item:
            return _dec(item[key])
    return Decimal("0")


def _ensure_futures_margin(client: Client, required: Decimal = Decimal("0.5")) -> Decimal:
    if _futures_flex_available(client) >= required:
        return Decimal("0")
    needed = required - _futures_flex_available(client)
    amount = needed.quantize(Decimal("0.00000001"), rounding=ROUND_UP)
    if _futures_cash_available(client, "usdt") < amount:
        pytest.fail("Insufficient Kraken Futures cash USDT to fund flex margin.", pytrace=False)
    _assert_futures_ok(
        client.futures_wallet_transfer(
            amount=_fmt(amount),
            fromAccount="cash",
            toAccount="flex",
            unit="USDT",
        )
    )
    time.sleep(2)
    if _futures_flex_available(client) < required:
        pytest.fail(
            "Kraken Futures flex margin remains insufficient after transfer.", pytrace=False
        )
    return amount


def _return_futures_margin(client: Client, amount: Decimal) -> None:
    if amount <= 0:
        return
    transfer_amount = min(amount, _futures_flex_available(client)).quantize(
        Decimal("0.00000001"),
        rounding=ROUND_DOWN,
    )
    if transfer_amount <= 0:
        return
    _assert_futures_ok(
        client.futures_wallet_transfer(
            amount=_fmt(transfer_amount),
            fromAccount="flex",
            toAccount="cash",
            unit="USDT",
        )
    )


def _futures_position_size(client: Client) -> Decimal:
    response = _assert_futures_ok(client.get_futures_open_positions())
    positions = response.get("openPositions") or response.get("positions") or []
    total = Decimal("0")
    if not isinstance(positions, list):
        return total
    for item in positions:
        if not isinstance(item, dict):
            continue
        if item.get("symbol") and item.get("symbol") != FUTURES_EXCHANGE_SYMBOL:
            continue
        for key in ("size", "qty", "quantity"):
            if key in item:
                total += _dec(item[key])
                break
    return total


def _skip_if_futures_state(client: Client) -> None:
    _cleanup_futures_state(client)


def _futures_min_size(client: Client) -> Decimal:
    details = client.ptm.get_trading_details("kraken", FUTURES_SYMBOL)
    step = _dec(details.get("size_precision"), "0.0001")
    min_size = max(_dec(details.get("min_size"), "0.0001"), step)
    return _round_to_step(min_size, step, ROUND_UP)


def _futures_order_params(client: Client, side: str) -> tuple[str, str]:
    tick = Decimal("0.5")
    book = _assert_futures_ok(client.get_futures_orderbook(FUTURES_SYMBOL)).get("orderBook", {})
    bids = book.get("bids", []) if isinstance(book, dict) else []
    asks = book.get("asks", []) if isinstance(book, dict) else []
    assert bids and asks
    if side == "buy":
        price = _round_to_step(_dec(bids[0][0]) - tick, tick, ROUND_DOWN)
    else:
        price = _round_to_step(_dec(asks[0][0]) + tick, tick, ROUND_UP)
    return _fmt(_futures_min_size(client)), _fmt(price)


def _futures_order_id(response) -> str:
    send_status = _assert_futures_ok(response).get("sendStatus", {})
    order_id = send_status.get("order_id") if isinstance(send_status, dict) else None
    assert order_id, response
    return str(order_id)


def _cancel_futures(client: Client, order_id: str) -> None:
    _assert_futures_ok(client.cancel_futures_order(order_id=order_id))
    time.sleep(0.5)


def _spot_product_for_btc_cleanup(client: Client) -> tuple[str, str]:
    for quote in SPOT_QUOTES:
        product_symbol = f"BTC-{quote}-SPOT"
        with suppress(Exception):
            client.ptm.get_exchange_symbol("kraken", product_symbol)
            _spot_prices(client, product_symbol)
            return product_symbol, quote
    pytest.fail("No Kraken BTC spot market is available for cleanup.", pytrace=False)


def _cleanup_spot_btc(client: Client, initial_btc: Decimal) -> None:
    current = _spot_available(client, "BTC")
    delta = current - initial_btc
    if delta <= 0:
        return

    product_symbol, quote = _spot_product_for_btc_cleanup(client)
    _, step, min_size, min_notional = _spot_details(client, product_symbol)
    sellable = _round_to_step(delta, step, ROUND_DOWN)
    if sellable <= 0:
        return

    bid, ask = _spot_prices(client, product_symbol)
    if sellable < min_size or sellable * bid < min_notional:
        target_base = max(min_size, min_notional / bid)
        buy_volume = _round_to_step(target_base - sellable + step, step, ROUND_UP)
        if buy_volume > 0:
            required_quote = buy_volume * ask * Decimal("1.02")
            if _spot_available(client, quote) < required_quote:
                pytest.fail(
                    "Insufficient Kraken quote balance to top up BTC dust for cleanup.",
                    pytrace=False,
                )
            _assert_spot_ok(client.place_spot_market_buy_order(product_symbol, _fmt(buy_volume)))
            time.sleep(3)
            sellable = _round_to_step(
                _spot_available(client, "BTC") - initial_btc, step, ROUND_DOWN
            )

    if sellable > 0:
        _assert_spot_ok(client.place_spot_market_sell_order(product_symbol, _fmt(sellable)))
        time.sleep(3)


def _cleanup_spot_state(client: Client, initial_btc: Decimal) -> None:
    open_orders = _assert_spot_ok(client.get_spot_open_orders()).get("result", {}).get("open", {})
    if open_orders:
        _assert_spot_ok(client.cancel_spot_all_orders())
        time.sleep(1)
    _cleanup_spot_btc(client, initial_btc)

    open_orders = _assert_spot_ok(client.get_spot_open_orders()).get("result", {}).get("open", {})
    if open_orders:
        pytest.fail("Kraken spot still has open orders after cleanup.", pytrace=False)
    if _spot_available(client, "BTC") - initial_btc > Decimal("0"):
        product_symbol, _ = _spot_product_for_btc_cleanup(client)
        _, step, _, _ = _spot_details(client, product_symbol)
        if _spot_available(client, "BTC") - initial_btc > step:
            pytest.fail("Kraken BTC spot balance still exists after cleanup.", pytrace=False)


def _cleanup_futures_state(client: Client) -> None:
    open_orders = _assert_futures_ok(client.get_futures_open_orders()).get("openOrders", [])
    if open_orders:
        _assert_futures_ok(client.cancel_futures_all_orders(product_symbol=FUTURES_SYMBOL))
        time.sleep(1)

    size = _futures_position_size(client)
    if size > 0:
        _assert_futures_ok(
            client.place_futures_market_sell_order(
                FUTURES_SYMBOL,
                _fmt(abs(size)),
                reduceOnly=True,
            )
        )
        time.sleep(2)
    elif size < 0:
        _assert_futures_ok(
            client.place_futures_market_buy_order(
                FUTURES_SYMBOL,
                _fmt(abs(size)),
                reduceOnly=True,
            )
        )
        time.sleep(2)

    if _assert_futures_ok(client.get_futures_open_orders()).get("openOrders", []):
        pytest.fail("Kraken Futures still has open orders after cleanup.", pytrace=False)
    if _futures_position_size(client) != 0:
        pytest.fail("Kraken Futures BTC position still exists after cleanup.", pytrace=False)


def _cleanup_state(client: Client) -> None:
    _cleanup_spot_state(client, Decimal("0"))
    _cleanup_futures_state(client)


def _is_kraken_service_unavailable(exc: FailedRequestError) -> bool:
    return str(exc.status_code) == "503" or "ErrCode: 503" in str(exc)


def _wait_for_spot_floor(client: Client, currency: str, floor: Decimal) -> bool:
    for delay in (0, 1, 2, 3, 4, 5):
        if delay:
            time.sleep(delay)
        if _spot_available(client, currency) >= floor:
            return True
    return False


def _withdraw_futures_to_spot_safely(
    client: Client,
    *,
    amount: str,
    currency: str,
    sourceWallet: str,
    restored_spot_floor: Decimal | None = None,
) -> None:
    try:
        _assert_futures_ok(
            client.withdraw_futures_to_spot_wallet(
                amount=amount,
                currency=currency,
                sourceWallet=sourceWallet,
            )
        )
    except FailedRequestError as exc:
        if (
            not _is_kraken_service_unavailable(exc)
            or restored_spot_floor is None
            or not _wait_for_spot_floor(client, currency, restored_spot_floor)
        ):
            raise
        return

    if restored_spot_floor is not None:
        assert _wait_for_spot_floor(client, currency, restored_spot_floor), (
            f"Kraken spot {currency} balance did not recover after Futures withdrawal."
        )


def test_wallet_transfer_round_trip(client):
    initial_spot = _spot_available(client, "USDT")
    if initial_spot < SPOT_TRANSFER_AMOUNT:
        pytest.fail(
            "Insufficient Kraken spot USDT for Spot-to-Futures transfer round-trip.",
            pytrace=False,
        )

    transferred = False
    try:
        _assert_spot_ok(
            client.wallet_transfer_to_futures(
                asset="USDT",
                amount=_fmt(SPOT_TRANSFER_AMOUNT),
                from_="Spot Wallet",
                to="Futures Wallet",
            )
        )
        transferred = True
        time.sleep(2)
        transferred = False
        _withdraw_futures_to_spot_safely(
            client,
            amount=_fmt(SPOT_TRANSFER_AMOUNT),
            currency="USDT",
            sourceWallet="flex",
            restored_spot_floor=initial_spot,
        )
    finally:
        if transferred:
            _withdraw_futures_to_spot_safely(
                client,
                amount=_fmt(SPOT_TRANSFER_AMOUNT),
                currency="USDT",
                sourceWallet="flex",
                restored_spot_floor=initial_spot,
            )
        time.sleep(2)


def test_futures_internal_transfer_round_trip(client):
    if _futures_flex_available(client) >= FUTURES_TRANSFER_AMOUNT:
        from_account, to_account = "flex", "cash"
    elif _futures_cash_available(client, "usdt") >= FUTURES_TRANSFER_AMOUNT:
        from_account, to_account = "cash", "flex"
    else:
        pytest.fail(
            "Insufficient Kraken Futures USDT for internal transfer round-trip.",
            pytrace=False,
        )

    transferred = False
    _assert_futures_ok(
        client.futures_wallet_transfer(
            amount=_fmt(FUTURES_TRANSFER_AMOUNT),
            fromAccount=from_account,
            toAccount=to_account,
            unit="USDT",
        )
    )
    transferred = True
    try:
        time.sleep(2)
    finally:
        if transferred:
            _assert_futures_ok(
                client.futures_wallet_transfer(
                    amount=_fmt(FUTURES_TRANSFER_AMOUNT),
                    fromAccount=to_account,
                    toAccount=from_account,
                    unit="USDT",
                )
            )
            transferred = False

    _assert_futures_ok(client.get_futures_accounts())


def test_spot_post_only_order_lifecycle(client):
    _skip_if_spot_open_orders(client)
    product_symbol, volume, price = _spot_limit_buy_params(client)

    creators = (
        lambda: client.place_spot_limit_order(product_symbol, "buy", volume, price),
        lambda: client.place_spot_limit_buy_order(product_symbol, volume, price),
        lambda: client.place_spot_post_only_limit_order(product_symbol, "buy", volume, price),
        lambda: client.place_spot_post_only_limit_buy_order(product_symbol, volume, price),
    )
    for create_order in creators:
        txid = None
        try:
            txid = _spot_txid(create_order())
            _assert_spot_ok(client.get_spot_orders(txid=txid))
            assert _assert_spot_ok(client.get_spot_open_orders()).get("result", {}).get("open")
        finally:
            if txid is not None:
                _cancel_spot(client, txid)


def test_spot_cancel_all_orders(client):
    _skip_if_spot_open_orders(client)
    product_symbol, volume, price = _spot_limit_buy_params(client)
    txid = None
    try:
        txid = _spot_txid(
            client.place_spot_post_only_limit_buy_order(product_symbol, volume, price)
        )
        _assert_spot_ok(client.cancel_spot_all_orders())
        txid = None
        time.sleep(1)
        open_orders = client.get_spot_open_orders().get("result", {}).get("open", {})
        assert not open_orders
    finally:
        if txid is not None:
            _cancel_spot(client, txid)


def test_spot_market_round_trip_and_sell_wrappers(client):
    _skip_if_spot_open_orders(client)
    product_symbol, _, step, min_size, volume = _spot_symbol_with_funds(client)
    before = _spot_available(client, "BTC")
    initial_btc = before
    acquired = Decimal("0")
    try:
        _assert_spot_ok(client.place_spot_market_order(product_symbol, "buy", _fmt(volume)))
        time.sleep(3)
        acquired = _round_to_step(_spot_available(client, "BTC") - before, step, ROUND_DOWN)
        assert acquired >= min_size

        sell_price = _spot_high_sell_price(client, product_symbol)
        for create_order in (
            lambda: client.place_spot_limit_sell_order(product_symbol, _fmt(acquired), sell_price),
            lambda: client.place_spot_post_only_limit_sell_order(
                product_symbol,
                _fmt(acquired),
                sell_price,
            ),
        ):
            txid = None
            try:
                txid = _spot_txid(create_order())
            finally:
                if txid is not None:
                    _cancel_spot(client, txid)

        _assert_spot_ok(client.place_spot_market_sell_order(product_symbol, _fmt(acquired)))
        acquired = Decimal("0")
        time.sleep(3)

        before = _spot_available(client, "BTC")
        _assert_spot_ok(client.place_spot_market_buy_order(product_symbol, _fmt(volume)))
        time.sleep(3)
        acquired = _round_to_step(_spot_available(client, "BTC") - before, step, ROUND_DOWN)
        assert acquired >= min_size
        _assert_spot_ok(client.place_spot_market_order(product_symbol, "sell", _fmt(acquired)))
        acquired = Decimal("0")
    finally:
        _cleanup_spot_btc(client, initial_btc)

    _assert_spot_ok(client.get_spot_closed_orders())
    _assert_spot_ok(client.get_spot_trade_history())


def test_futures_post_only_order_lifecycle(client):
    _skip_if_futures_state(client)
    transferred = _ensure_futures_margin(client)
    try:
        buy_size, buy_price = _futures_order_params(client, "buy")
        for create_order in (
            lambda: client.place_futures_limit_order(FUTURES_SYMBOL, "buy", buy_size, buy_price),
            lambda: client.place_futures_limit_buy_order(FUTURES_SYMBOL, buy_size, buy_price),
            lambda: client.place_futures_post_only_limit_order(
                FUTURES_SYMBOL,
                "buy",
                buy_size,
                buy_price,
            ),
            lambda: client.place_futures_post_only_limit_buy_order(
                FUTURES_SYMBOL,
                buy_size,
                buy_price,
            ),
        ):
            order_id = None
            try:
                order_id = _futures_order_id(create_order())
                _assert_futures_ok(client.get_futures_order_status(orderIds=[order_id]))
                _assert_futures_ok(client.get_futures_open_orders())
            finally:
                if order_id is not None:
                    _cancel_futures(client, order_id)

        sell_size, sell_price = _futures_order_params(client, "sell")
        for create_order in (
            lambda: client.place_futures_limit_sell_order(FUTURES_SYMBOL, sell_size, sell_price),
            lambda: client.place_futures_post_only_limit_sell_order(
                FUTURES_SYMBOL,
                sell_size,
                sell_price,
            ),
        ):
            order_id = None
            try:
                order_id = _futures_order_id(create_order())
            finally:
                if order_id is not None:
                    _cancel_futures(client, order_id)
    finally:
        _return_futures_margin(client, transferred)


def test_futures_cancel_all_orders(client):
    _skip_if_futures_state(client)
    transferred = _ensure_futures_margin(client)
    order_id = None
    try:
        size, price = _futures_order_params(client, "buy")
        order_id = _futures_order_id(
            client.place_futures_post_only_limit_buy_order(FUTURES_SYMBOL, size, price)
        )
        _assert_futures_ok(client.cancel_futures_all_orders(product_symbol=FUTURES_SYMBOL))
        order_id = None
        time.sleep(1)
        assert not _assert_futures_ok(client.get_futures_open_orders()).get("openOrders", [])
    finally:
        if order_id is not None:
            _cancel_futures(client, order_id)
        _return_futures_margin(client, transferred)


def test_futures_market_round_trip(client):
    _skip_if_futures_state(client)
    transferred = _ensure_futures_margin(client)
    size = _fmt(_futures_min_size(client))
    try:
        _assert_futures_ok(client.place_futures_market_order(FUTURES_SYMBOL, "buy", size))
        time.sleep(2)
        _assert_futures_ok(
            client.place_futures_market_sell_order(
                FUTURES_SYMBOL,
                size,
                reduceOnly=True,
            )
        )
        time.sleep(2)
        assert _futures_position_size(client) == 0

        _assert_futures_ok(client.place_futures_market_buy_order(FUTURES_SYMBOL, size))
        time.sleep(2)
        _assert_futures_ok(
            client.place_futures_market_order(
                FUTURES_SYMBOL,
                "sell",
                size,
                reduceOnly=True,
            )
        )
        time.sleep(2)
        assert _futures_position_size(client) == 0
    finally:
        try:
            _cleanup_futures_state(client)
        finally:
            _return_futures_margin(client, transferred)
