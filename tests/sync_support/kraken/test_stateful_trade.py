...
def _spot_symbol_with_funds(client: Client) -> tuple[str, str, Decimal, Decimal, Decimal]:
    for quote in SPOT_QUOTES:
        ...
        with suppress(Exception):
            ...
            if _spot_available(client, quote) >= volume * ask * Decimal("1.05"):
...
def test_spot_market_round_trip_and_sell_wrappers(client):
    ...
    try:
        ...
        leftover = _round_to_step(_spot_available(client, "BTC") - before, step, ROUND_DOWN)
        cleanup_size = max(leftover, acquired)
        if cleanup_size >= min_size:
            with suppress(Exception):
                client.place_spot_market_sell_order(product_symbol, _fmt(cleanup_size))
...
def test_futures_market_round_trip(client):
    ...
    try:
        _assert_futures_ok(
            client.place_futures_market_order(FUTURES_SYMBOL, "buy", _fmt(FUTURES_SIZE))
        )
        time.sleep(2)
        _assert_futures_ok(
        ...
        _assert_futures_ok(
            client.place_futures_market_buy_order(FUTURES_SYMBOL, _fmt(FUTURES_SIZE))
        )
        time.sleep(2)
        _assert_futures_ok(
