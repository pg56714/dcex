...
def test_spot_post_only_order_lifecycle(client):
    ...
    try:
        ...
        assert client.get_order(product_symbol=SPOT_SYMBOL, orderId=order_id)["orderId"] == order_id
        assert isinstance(client.get_open_orders(product_symbol=SPOT_SYMBOL), list)
    finally:
        if order is not None:
            client.cancel_order(product_symbol=SPOT_SYMBOL, orderId=int(order["orderId"]))
...
def test_spot_market_order_round_trip(client):
    ...
    btc_after = _spot_free(client, "BTC")
