...
def _has_lighter_sdk() -> bool:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        return find_spec("lighter") is not None and find_spec("lighter.signer_client") is not None


HAS_LIGHTER_SDK = _has_lighter_sdk()

pytestmark = [
    pytest.mark.private,
    pytest.mark.skipif(
        not HAS_LIGHTER_SDK,
        reason="Install lighter-sdk to run Lighter private signing tests.",
    ),
]
...
def test_auth_token_and_key_check(client):
    token = client.create_auth_token()
    assert isinstance(token, str)
    assert token
    assert client.check_client() is None
...
def test_private_account_reads(client):
    ...
    _assert_response(client.get_position_funding(limit=5))
    _assert_response(client.get_maker_only_api_keys())
    _assert_response(client.get_tokens(account_index=account_index))
    _assert_response(client.get_public_pools_metadata(index=0, limit=5))
    _assert_response(
        client.get_pnl(
            by="index",
    ...
    _assert_response(client.get_trades(account_index=account_index, sort_by="timestamp", limit=5))
    _assert_response(client.get_next_nonce())
...
def test_private_bridge_and_history_reads(client):
    ...
    _assert_response(client.get_l1_metadata(l1_address=l1_address))
    _assert_response(client.get_deposit_history(l1_address=l1_address, filter="all"))
    _assert_response(client.get_fastwithdraw_info())
    _assert_response(client.get_transfer_history())
    _assert_response(client.get_transfer_fee_info())
    _assert_response(client.get_withdraw_history(filter="all"))


@pytest.mark.generated
def test_private_export_read(client):
...
def test_private_referral_lease_reads(client):
    ...
    _assert_response(client.get_referral_points())
    _assert_response(client.get_referral_user_referrals(l1_address=l1_address, limit=5))
    _assert_response(client.get_leases(limit=5))
    _assert_response(client.get_partner_stats())
