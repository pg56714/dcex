...
async def _balances(client: Client) -> dict:
    response = await client.get_balances()
    assert isinstance(response, dict)
    return response


async def _available(client: Client, asset: str) -> Decimal:
    balance = (await _balances(client)).get(asset, {})
    if isinstance(balance, dict):
        return _dec(balance.get("available"))
    return Decimal("0")


async def _lent(client: Client, asset: str) -> Decimal:
    positions = _items(await client.get_borrow_lend_positions())
...
async def _total_asset(client: Client, asset: str) -> Decimal:
    return await _available(client, asset) + await _lent(client, asset)


async def _market_details(client: Client, symbol: str) -> tuple[Decimal, Decimal, Decimal]:
    market = await client.get_market(symbol)
...
async def _book_prices(client: Client, symbol: str) -> tuple[Decimal, Decimal]:
    book = await client.get_order_book_depth(symbol, limit=5)
...
async def _safe_limit_price(client: Client, symbol: str, side: str) -> str:
...
async def _min_quantity(client: Client, symbol: str) -> Decimal:
    _, step, min_size = await _market_details(client, symbol)
...
async def _open_orders(client: Client, symbol: str) -> list[dict]:
    return _items(await client.get_open_orders(product_symbol=symbol))


async def _position_size(client: Client, symbol: str) -> Decimal:
    positions = _items(await client.get_open_positions())
    size = Decimal("0")
    for position in positions:
...
async def _cancel_order(client: Client, symbol: str, order_id: str) -> None:
    await client.cancel_order(symbol, orderId=order_id)
    await asyncio.sleep(1)


async def _cancel_all_symbol_orders(client: Client, symbol: str) -> None:
    with suppress(Exception):
        await client.cancel_open_orders(product_symbol=symbol)
    await asyncio.sleep(1)
...
async def test_spot_stateful_order_lifecycle(client):
    ...
    try:
        ...
        acquired = await _total_asset(client, "SOL") - initial_sol
        _, step, _ = await _market_details(client, SPOT_SYMBOL)
        ...
        order_id = _order_id(
            await client.place_limit_order(
                ...
                price=await _safe_limit_price(client, SPOT_SYMBOL, "Ask"),
                postOnly=True,
                autoLendRedeem=True,
            )
        )
        assert isinstance(await client.get_open_order(SPOT_SYMBOL, orderId=order_id), dict)
        await _cancel_order(client, SPOT_SYMBOL, order_id)
        ...
        assert isinstance(
            await client.get_order_history(product_symbol=SPOT_SYMBOL, limit=20),
            list,
        )
    ...
    try:
        ...
        order_id = _order_id(
            await client.place_limit_order(
                ...
                side="Ask",
                quantity=_fmt(qty),
                price=await _safe_limit_price(client, PERP_SYMBOL, "Ask"),
                postOnly=True,
                reduceOnly=True,
            )
        )
        assert isinstance(await client.get_open_order(PERP_SYMBOL, orderId=order_id), dict)
        await _cancel_order(client, PERP_SYMBOL, order_id)
        ...
        assert await _position_size(client, PERP_SYMBOL) == 0
        assert isinstance(
            await client.get_order_history(product_symbol=PERP_SYMBOL, limit=20),
            list,
        )
